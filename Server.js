...
app.post('/api/notion/databas1', async (req, res) => {
  try {
    const response = await axios.post(
      `${NOTION_API_BASE_URL}/databases/587b3851ebb4484086716104ce9e5d4d/query`,
      req.body,
      {
        headers: {
...
app.patch('/api/notion/timespan', async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: 'Failed to update timespan', error: error.message });
  }
});
////////////////
...
app.post('/api/notion/databas2', async (req, res) => {
  try {
    const response = await axios.post(
      `${NOTION_API_BASE_URL}/databases/7c27b5453a7a49e5b4314926fc51b85d/query`,
      req.body,
      {
        headers: {
          Authorization: `Bearer ${NOTION_API_KEY}`,
...
app.post('/api/notion/databas3', async (req, res) => {
  try {
    const response = await axios.post(
      `${NOTION_API_BASE_URL}/databases/e9d6cc1e1cd240a9b7f8c160921358e5/query`,
      req.body,
      {
        headers: {
          Authorization: `Bearer ${NOTION_API_KEY}`,
...
app.post('/getdatabasebyid', async (req, res) => {
  ...
  const notion = new Client({
    auth: 'secret_rjSOXoc32cwyEA6cf6E2queEs9Jk74iOvlxj9p6w61i',
  });
  ...
  try {
    ...
    const filteredResults = response.results.filter((page) => {
      ...
      return (
        page.properties.PrivateID &&
        page.properties.PrivateID.rich_text.length > 0 &&
        page.properties.PrivateID.rich_text[0].text.content === privateID
      );
    });
...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (response.results.length > 0) {
      const userPrivateID =
        response.results[0].properties.PrivateID.rich_text[0].text.content;

      res
        .status(200)
        .json({ message: 'Autentisering lyckades', privateID: userPrivateID });
    } else {
